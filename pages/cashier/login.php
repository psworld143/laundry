    ...
    if (empty($username) || empty($password)) {
        ...
        try {
            ...
            if ($user && password_verify($password, $user['password_hash'])) {
                ...
                if (in_array($user['position'], ['cashier', 'admin']) || $user['position'] === 'admin') {
                    ...
                    $stmt = $db->prepare("
                        ...
                        VALUES (?, 'login', 'Cashier login successful', ?, ?)
                    ");
                    $stmt->execute([
                        $user['user_id'],
                    ...
                    redirect('pages/cashier/dashboard.php');
                } else {
                    $error = 'Access denied. Cashier access required.';
                }
            } else {
...
<head>
    ...
    <title><?php echo APP_NAME; ?> - Cashier Login</title>
...
<body class="login-bg min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-white opacity-80">Cashier Login Portal</p>
        </div>
